...
export function PlanView({snapshot,index=0,planId}:{snapshot:Snapshot;index?:number;planId?:string}) {
  ...
  return <article className="plan">
    ...
    <dl className="scores"><div><dt>必要休暇</dt><dd>{plan.score.vacationDays}<small>日</small></dd></div><div><dt>鑑賞日数</dt><dd>{plan.score.screeningDays}<small>日</small></dd></div><div><dt>鑑賞作品</dt><dd>{plan.screenings.length}<small>作品</small></dd></div><div><dt>移動時間</dt><dd>{plan.score.travelMinutes}<small>分</small></dd></div><div><dt>待ち時間</dt><dd>{plan.score.waitingMinutes}<small>分</small></dd></div></dl>
