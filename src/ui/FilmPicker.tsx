...
const departments = [...new Set(festival.films.map(f => f.department).filter((d): d is string => !!d))];
...
export function FilmPicker({ selected, onChange }: { selected: string[]; onChange: (ids: string[]) => void }) {
  ...
  const resetFilters = () => { setQuery(''); setDepartment(''); setDate(''); setOnlySelected(false); setPage(1); };
  ...
  return <section aria-labelledby="films-heading">
    ...
    <div className="film-filters">
      ...
      <label>部門で絞り込み<select value={department} onChange={e => { setDepartment(e.target.value); setPage(1); }}><option value="">すべての部門・企画</option>{departments.map(d => <option key={d} value={d}>{d}（{festival.films.filter(f => f.department === d).length}作品）</option>)}</select></label>
    ...
    <div className="film-selection-tools">
      ...
      <button type="button" onClick={() => { setQuery(''); setDepartment(''); setDate(''); setOnlySelected(true); setPage(1); }}>選んだ{selected.length}作品を確認</button>
      {(query || department || date || onlySelected) && <button type="button" onClick={resetFilters}>絞り込みを解除</button>}
    </div>
    {date && <p className="hint">上映日は作品を探すための絞り込みです。スケジュールは全日程から生成します。</p>}
    <p className="film-count" role="status">{matching.length ? `${matching.length}作品中 ${(currentPage - 1) * PAGE_SIZE + 1}–${Math.min(currentPage * PAGE_SIZE, matching.length)}作品を表示` : onlySelected && !selected.length ? 'まだ作品を選択していません。' : '該当する作品はありません。検索語・部門・上映日を変更してください。'}</p>
    <div className="films">{visible.map(f => {
      ...
      return <article className={`film-card ${checked ? 'selected' : ''}`} key={f.id}>
        ...
        <p className="film-department">{f.department}</p>
