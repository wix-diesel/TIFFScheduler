...
export default function App() {
  ...
  return <><header><span className="brand">TIFF <span>Scheduler</span></span><span className="tag">TIFF 2025</span></header><main><div className="intro"><p className="eyebrow">映画祭の鑑賞プラン</p><h1>観たい映画から、<br className="mobile-break"/>一日の予定へ。</h1><p className="notice">2025年10月27日〜11月5日の公式データを使用しています。2026年の予定ではありません。終了はイベント等を含む確保時間、移動は余裕を持った推定です。休暇判定は設定した勤務曜日を優先し、追加休日は日付単位で反映します。</p><details className="data-details"><summary>データの出典と収録範囲</summary><p><a href={festivalInfo.sourceUrl} target="_blank" rel="noreferrer">第38回東京国際映画祭 公式サイト</a> · 確認日 {festivalInfo.retrievedAt.slice(0,10)}</p><p>{festival.films.length}作品・{festival.screenings.length}上映を収録。本編時間不明の企画・受賞作品未確定枠など{festivalInfo.excludedCount}件は対象外です。終了時刻の不整合{festivalInfo.correctionCount}件は本編時間以上を確保するよう補正しています。</p><p>移動はJR有楽町駅経由を想定し、建物内移動と信号待ちを含めた目安です。入退場の余裕はさらに加算します。チケットの取得可否や当日の変更・延長は反映しません。</p></details></div>
    ...
    <div className="generate-bar"><span><strong>{selected.length}作品</strong>を選択中 <small>鑑賞数 → 休暇日数 → 鑑賞日数 → 移動 → 待ち時間の順に優先</small></span><button className="primary" disabled={!selected.length} type="submit">スケジュールを生成</button></div></form>
