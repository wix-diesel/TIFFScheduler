...
現在は **Phase 4: Deployment** の公開ワークフローとブラウザテストまで実装済みです。2025年の公式データ（149作品・282上映・13会場）で、作品検索・選択、1日最大鑑賞本数・上映開始下限・上映終了上限を含む条件設定、最大3案の生成、日別タイムライン、設定の自動復元、名前付きプラン保存を利用できます。
...
- [1日最大鑑賞本数の設計](docs/max-screenings-per-day-design.md)
- [上映開始時刻下限の設計](docs/earliest-screening-start-design.md)
- [上映終了時刻上限の設計](docs/latest-screening-end-design.md)
...
「最大上映本数」は空欄（初期値）なら制限なし、1以上の整数なら全日共通の上限です。上映開始時刻の日本時間の日付で数え、上限内で全作品を鑑賞できない場合も、鑑賞数が最大になる代替案を表示します。

「上映開始時刻」は空欄（初期値）なら制限なしです。指定時刻以降に始まる上映だけを候補にし、上映前イベントがある場合はその開始時刻で判定します。入場の余裕時間はこの条件には含めません。

「上映終了時刻」は空欄（初期値）なら制限なしです。指定時刻までに終わる上映だけを候補にし、上映後イベントがある場合はその終了時刻で判定します。退場の余裕時間や自宅への帰路はこの条件に含めません。
