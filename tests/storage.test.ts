...
test('round trip preserves all settings and independent snapshots',()=>{
 const original=state(); original.constraints.maxScreeningsPerDay=2;original.constraints.earliestScreeningStart='10:00'; const restored=decode(encode(original),'tiff-2025');assert.deepEqual(restored,original);
 assert.equal(restored.constraints.maxScreeningsPerDay,2);
 assert.equal(restored.constraints.earliestScreeningStart,'10:00');
...
test('20 plan limit, deletion and invalid numeric drafts',()=>{
 ...
 for(const value of ['', '9:00', '24:00']){const invalid=state();invalid.constraints.earliestScreeningStart=value;assert.throws(()=>decode(JSON.stringify(invalid),'tiff-2025'));}
});
